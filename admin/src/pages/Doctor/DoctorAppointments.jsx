      ...
      <div className="bg-white p-5 rounded-lg shadow-sm mb-6">
        <div className="w-full mb-4">
          <div className="relative">
            ...
            />
            <svg className="absolute right-3 top-3.5 h-5 w-5 text-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
          ...
          {filteredAppointments.length > 0 ? filteredAppointments.map((item, index) => (
            <div key={item._id} className="border-b last:border-b-0">
              <div 
                ...
                className={`flex flex-wrap sm:grid grid-cols-[0.5fr_2fr_1fr_2fr_1fr_1.5fr] gap-2 items-center py-4 px-6 hover:bg-indigo-600/5 cursor-pointer transition-colors ${expandedId === item._id ? 'bg-indigo-600/10' : ''}`}
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600 sm:hidden">Age: {calculateAge(item.userData.dob)}</p>
                  </div>
                </div>
                ...
                <span className="text-gray-900 font-medium">{currency} {item.amount}</span>
                <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                  {item.cancelled ? (
                    <span className="px-3 py-1.5 text-xs font-medium rounded-full bg-red-100 text-red-600">Cancelled</span>
                  ) : item.isCompleted ? (
                    <span className="px-3 py-1.5 text-xs font-medium rounded-full bg-green-100 text-green-600">Completed</span>
                  ) : (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="space-y-2">
                        <p className="text-sm"><span className="text-gray-600 font-medium">Email:</span> {item.userData.email || 'N/A'}</p>
                        <p className="text-sm"><span className="text-gray-600 font-medium">Phone:</span> {item.userData.phone || 'N/A'}</p>
                        <p className="text-sm"><span className="text-gray-600 font-medium">Gender:</span> {item.userData.gender || 'N/A'}</p>
                      </div>
                    </div>
                    <div>
